...
import {
  LayoutDashboard, Inbox, CheckCircle, XCircle, FolderTree,
  Tv2, Tag, Settings, Search, Bell, ChevronLeft, Menu, User,
} from "lucide-react";
...
const navItems = [
  ...
  { label: "Video Inbox", icon: Inbox, path: "/inbox" },
  { label: "Published Videos", icon: CheckCircle, path: "/published" },
  { label: "Rejected / Archived", icon: XCircle, path: "/rejected" },
  { divider: true },
...
export default function AppLayout({ children }: { children: ReactNode }) {
  ...
  const pageTitle =
    ...
    (currentPath.startsWith("/inbox/") ? "Video Review" : "Digdaya Video CMS");
        ...
        <div className="flex h-16 items-center gap-3 border-b border-border px-4">
          ...
          {!collapsed && (
            <div className="overflow-hidden">
              ...
              <p className="text-[11px] text-muted-foreground">Video Management</p>
            </div>
          )}
        </div>
