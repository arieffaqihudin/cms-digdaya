...
import { Search, Filter, ChevronDown, Eye, CheckCircle, XCircle, FolderTree, Tag } from "lucide-react";
...
export default function VideoInbox() {
      ...
      <div className="rounded-lg border border-border bg-surface p-4 shadow-card">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Select value={channelFilter} onValueChange={setChannelFilter}>
            <SelectTrigger className="w-[160px] h-9 rounded-md">
      ...
      {selected.length > 0 && (
        <div className="flex items-center gap-3 rounded-lg border border-primary/20 bg-primary/5 p-3">
          ...
          <div className="flex gap-2 ml-auto">
            <Button size="sm" className="h-8 rounded-md bg-primary text-primary-foreground hover:bg-primary/90">
              <CheckCircle className="h-3.5 w-3.5 mr-1.5" /> Publish Selected
            </Button>
            <Button size="sm" variant="outline" className="h-8 rounded-md">
              <XCircle className="h-3.5 w-3.5 mr-1.5" /> Reject Selected
            </Button>
            <Button size="sm" variant="outline" className="h-8 rounded-md">
              <FolderTree className="h-3.5 w-3.5 mr-1.5" /> Assign Category
            </Button>
            <Button size="sm" variant="outline" className="h-8 rounded-md">
              <Tag className="h-3.5 w-3.5 mr-1.5" /> Add Tags
            </Button>
          </div>
        </div>
      )}
                ...
                <tr key={v.id} className="hover:bg-accent/50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-medium text-foreground line-clamp-2 max-w-xs">
                        {v.title}
                      </span>
                    </div>
                  </td>
