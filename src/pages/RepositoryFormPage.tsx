...
import { ArrowLeft, Save, Upload, X, ImageIcon, Eye, CheckCircle, FileText } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-[12px] border border-border bg-card p-4 md:px-6 md:py-4 shadow-card">
        <div className="flex flex-wrap items-center gap-3 md:gap-4">
          <button onClick={() => navigate("/repository")} className="flex h-8 w-8 items-center justify-center rounded-[10px] text-muted-foreground hover:text-foreground hover:bg-accent transition-colors shrink-0">
          ...
          <div className="flex items-center gap-2 min-w-0 flex-1">
            ...
            <h1 className="text-sm md:text-[15px] font-semibold text-foreground truncate">Upload File Baru</h1>
          </div>
        ...
        <div className="space-y-5">
          <div className="rounded-[12px] border border-border bg-surface p-4 md:p-5 shadow-card space-y-4">
            <div className="space-y-1.5">
              ...
              <Input value={title} onChange={(e) => handleTitleChange(e.target.value)} placeholder="Masukkan nama file..." className="h-11 rounded-[10px] text-[14px] border-border focus-visible:ring-1 focus-visible:ring-ring" />
            </div>
          </div>
          ...
          <div className="rounded-[12px] border border-border bg-surface p-4 md:p-5 shadow-card space-y-4">
            ...
            <Textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Deskripsi singkat tentang file ini..." className="min-h-[120px] rounded-[10px] text-[13px] border-border focus-visible:ring-1 focus-visible:ring-ring resize-none" />
          </div>
        </div>
          ...
          <div className="rounded-[12px] border border-border bg-surface p-4 md:p-5 shadow-card space-y-5">
            <div className="space-y-1.5">
              ...
              <Select value={product} onValueChange={setProduct}>
                <SelectTrigger className="w-full h-10 rounded-[10px] text-[13px] border-border"><SelectValue placeholder="Pilih produk" /></SelectTrigger>
            ...
            <div className="space-y-1.5">
              ...
              <Select value={category} onValueChange={setCategory}>
                <SelectTrigger className="w-full h-10 rounded-[10px] text-[13px] border-border"><SelectValue placeholder="Pilih kategori" /></SelectTrigger>
                <SelectContent>{categories.map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div className="space-y-1.5">
              <Label className="text-[12px] font-medium text-muted-foreground uppercase tracking-[0.06em]">Slug</Label>
              <Input value={slug} onChange={(e) => setSlug(e.target.value)} placeholder="auto-generated-slug" className="h-10 rounded-[10px] text-[12px] font-mono border-border bg-accent/30 focus-visible:ring-1 focus-visible:ring-ring" />
            </div>
            <div className="space-y-1.5">
