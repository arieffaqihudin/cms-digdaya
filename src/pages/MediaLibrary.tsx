...
import { Search, Plus, Image, FileText, Video, Music, Trash2, Download, Eye, Grid3X3, List } from "lucide-react";
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[13px] text-muted-foreground mt-0.5">Kelola gambar, dokumen, dan file media</p>
        </div>
      ...
      <div className="rounded-[12px] border border-border bg-surface p-4 shadow-card">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center rounded-[10px] border border-border bg-background p-0.5">
            {[
      ...
      <div className="flex gap-5">
        ...
        <div className="flex-1 min-w-0">
          {/* Upload area */}
          <div className="rounded-[12px] border border-dashed border-border/60 bg-surface/50 p-8 text-center hover:border-primary/30 transition-colors cursor-pointer group mb-5">
          ...
          {view === "grid" ? (
                  ...
                  >
                    {item.type === "image" ? (
                      ...
                      <div className="aspect-[4/3] bg-accent/40 flex items-center justify-center">
                        <div className={cn("flex h-12 w-12 items-center justify-center rounded-[12px]", typeColors[item.type])}>
            ...
            <div className="rounded-[12px] border border-border bg-surface shadow-card overflow-hidden">
              <table className="w-full text-[13px]">
                <thead>
                  <tr className="border-b border-border/60">
                    <th className="px-5 py-3.5 text-left text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground">File</th>
                    <th className="px-5 py-3.5 text-left text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground">Tipe</th>
                    <th className="px-5 py-3.5 text-left text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground">Ukuran</th>
                    <th className="px-5 py-3.5 text-left text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground">Tanggal</th>
                  ...
                  {filtered.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-5 py-3.5 text-muted-foreground capitalize">{item.type === "image" ? "Gambar" : item.type === "document" ? "Dokumen" : item.type}</td>
                        <td className="px-5 py-3.5 text-muted-foreground tabular-nums">{item.size}</td>
                        <td className="px-5 py-3.5 text-muted-foreground tabular-nums">{item.date}</td>
              ...
              {filtered.length === 0 && (
                <div className="p-16 text-center text-[13px] text-muted-foreground">Tidak ada file ditemukan.</div>
              )}
            </div>
          )}
        </div>
