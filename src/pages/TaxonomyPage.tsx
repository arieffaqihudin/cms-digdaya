  ...
  return (
    <div className="space-y-4 md:space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-base md:text-lg font-semibold text-foreground">{config.label}</h1>
      ...
      <div className="rounded-[12px] border border-border bg-surface shadow-card overflow-hidden">
              ...
              <div key={item.id} className="p-4 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className="inline-flex items-center rounded-full bg-muted px-2 py-[2px] text-[10px] font-medium tabular-nums text-muted-foreground">
                      {item.count} konten
                    </span>
                    <span className={`inline-flex items-center gap-1 rounded-full px-2 py-[2px] text-[10px] font-medium ${
                      item.status === "active" ? "bg-status-success-bg text-status-success-fg" : "bg-muted text-muted-foreground"
                    }`}>
                      <span className={`h-1 w-1 rounded-full ${item.status === "active" ? "bg-status-success-fg" : "bg-muted-foreground/50"}`} />
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="border-b border-border/60">
                ...
                <th className="px-4 md:px-5 py-3.5 text-left text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground hidden lg:table-cell">Tipe</th>
            ...
            <tbody>
              {filtered.map((item) => (
                <tr key={item.id} className="border-b border-border/30 hover:bg-accent/30 transition-colors duration-150">
                  ...
                  <td className="px-4 md:px-5 py-4 text-muted-foreground hidden lg:table-cell">{config.label}</td>
                  ...
                  <td className="px-4 md:px-5 py-4">
                    <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-[3px] text-[11px] font-medium ${
                      ...
                      <span className={`h-1.5 w-1.5 rounded-full shrink-0 ${item.status === "active" ? "bg-status-success-fg" : "bg-muted-foreground/50"}`} />
                      {item.status === "active" ? "Aktif" : "Nonaktif"}
                    </span>
                  </td>
        ...
        {filtered.length === 0 && (
          <div className="p-12 md:p-16 text-center text-[13px] text-muted-foreground">Tidak ada item ditemukan.</div>
        )}
      </div>
    </div>
  );
...
export default function TaxonomyPage({ defaultTab }: { defaultTab?: string }) {
  ...
  return (
    <div className="space-y-4 md:space-y-5">
      <Tabs defaultValue={defaultTab || "categories"} className="space-y-4 md:space-y-5">
        <TabsList className="bg-surface border border-border rounded-[12px] p-1 h-auto flex-wrap">
          <TabsTrigger value="categories" className="rounded-[10px] text-[12px] md:text-[13px] px-3 md:px-4 py-2 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">Kategori</TabsTrigger>
          <TabsTrigger value="tags" className="rounded-[10px] text-[12px] md:text-[13px] px-3 md:px-4 py-2 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">Tag</TabsTrigger>
          <TabsTrigger value="products" className="rounded-[10px] text-[12px] md:text-[13px] px-3 md:px-4 py-2 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">Produk</TabsTrigger>
          <TabsTrigger value="channels" className="rounded-[10px] text-[12px] md:text-[13px] px-3 md:px-4 py-2 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">Channel</TabsTrigger>
        </TabsList>
