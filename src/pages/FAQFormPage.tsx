...
import { ArrowLeft, Save, Link2, Eye, HelpCircle, Pin, CheckCircle } from "lucide-react";
...
const masterProducts = ["Digdaya Persuratan", "Digdaya Pesantren", "Siskader NU", "Digdaya Kepengurusan"];
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-[12px] border border-border bg-card p-4 md:px-6 md:py-4 shadow-card">
        <div className="flex flex-wrap items-center gap-3 md:gap-4">
          <Button variant="ghost" size="icon" onClick={() => navigate("/faq")} className="rounded-[10px] text-muted-foreground hover:text-foreground h-8 w-8 shrink-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-5 md:gap-6">
        ...
        <div className="space-y-5">
          <SectionCard>
            <Field label="Title">
              <Input placeholder="Masukkan pertanyaan FAQ..." value={title} onChange={(e) => setTitle(e.target.value)} className="rounded-[10px] border-border/60 focus-visible:ring-primary/30 text-sm h-11" />
            </Field>
          ...
          <SectionCard title="Konten Jawaban">
            <Field label="Content" hint="Tulis jawaban lengkap untuk pertanyaan ini">
              <Textarea placeholder="Tulis jawaban FAQ di sini..." value={content} onChange={(e) => setContent(e.target.value)} rows={12} className="rounded-[10px] border-border/60 focus-visible:ring-primary/30 text-sm resize-none leading-relaxed" />
            </Field>
          </SectionCard>
        </div>
        ...
        <div className="space-y-5 lg:sticky lg:top-4 self-start">
          <SectionCard title="Publikasi">
            ...
            <div className="flex items-center gap-2 rounded-[10px] border border-border/60 bg-muted/20 px-3 py-2.5 text-xs text-muted-foreground">
              ...
              {isPinned && (<><span className="text-border">·</span><Pin className="h-3 w-3" /><span>Pinned</span></>)}
            </div>
          </SectionCard>
          ...
          <SectionCard title="Klasifikasi">
            <Field label="Produk">
              <Select value={product} onValueChange={handleProductChange}>
                ...
                <SelectContent className="rounded-[10px]">{masterProducts.map((p) => <SelectItem key={p} value={p}>{p}</SelectItem>)}</SelectContent>
              </Select>
            </Field>
            <Field label="Kategori FAQ" hint={!product ? "Pilih produk terlebih dahulu" : undefined}>
              <Select value={category} onValueChange={setCategory} disabled={!product}>
                <SelectTrigger className="rounded-[10px] border-border/60 focus:ring-primary/30 text-sm disabled:opacity-50"><SelectValue placeholder={product ? "Pilih kategori..." : "Pilih produk terlebih dahulu"} /></SelectTrigger>
                <SelectContent className="rounded-[10px]">{availableCategories.map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
              </Select>
            </Field>
          </SectionCard>
            ...
            <div className="space-y-3 text-xs">
              {[
                ...
                <div key={l} className="flex justify-between">
                  ...
                  <span className="text-foreground/80 font-medium truncate max-w-[160px] text-right">{v || "—"}</span>
                </div>
              ))}
              <div className="flex justify-between">
                <span className="text-muted-foreground">Status</span>
                <span className={`font-medium ${isPublished ? "text-[hsl(var(--status-success-fg))]" : "text-muted-foreground"}`}>{isPublished ? "Published" : "Draft"}</span>
              </div>
            </div>
          </SectionCard>
        </div>
