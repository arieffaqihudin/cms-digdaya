...
import { Search, Eye, Plus, GripVertical, HelpCircle, Filter } from "lucide-react";
        ...
        <div className="flex flex-wrap items-center gap-2 md:gap-3">
          ...
          {screenSize === "mobile" ? (
            <>
              ...
              <Link to="/content/new?type=faq" className="w-full sm:w-auto">
      ...
      <div className="rounded-[12px] border border-border bg-surface shadow-card overflow-hidden">
        {screenSize === "mobile" ? (
          <div className="divide-y divide-border/30">
            {paged.map((f) => (
              <Link key={f.id} to={`/content/${f.id}?type=faq`} className="block p-4 hover:bg-accent/30 transition-colors">
                <p className="text-[13px] font-medium text-foreground leading-[1.4]">{f.question}</p>
                <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                  <span className="text-[11px] text-muted-foreground">{f.category}</span>
                  <span className="text-[11px] text-muted-foreground/50">·</span>
                  <span className="inline-flex items-center rounded-full bg-primary/[0.07] px-2 py-[2px] text-[10px] font-medium text-primary">{f.relatedProduct}</span>
                </div>
                <div className="flex items-center gap-2 mt-2">
                  <StatusBadge status={f.status} />
                  <span className="text-[10px] text-muted-foreground tabular-nums">#{f.order}</span>
                </div>
              </Link>
            ))}
          </div>
            ...
            <table className="w-full text-[13px]">
              <thead>
                <tr className="border-b border-border/60">
                  <th className="px-3 py-3.5 w-8 hidden lg:table-cell"></th>
                  <th className="px-4 md:px-5 py-3.5 text-left text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground">Pertanyaan</th>
                  ...
                  <th className="px-4 md:px-5 py-3.5 text-center text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground hidden xl:table-cell">Urutan</th>
              ...
              <tbody>
                {paged.map((f) => (
                  <tr key={f.id} className="border-b border-border/30 hover:bg-accent/30 transition-colors duration-150">
                    <td className="px-3 py-4 hidden lg:table-cell"><GripVertical className="h-4 w-4 text-muted-foreground/40 cursor-grab" strokeWidth={1.6} /></td>
                    <td className="px-4 md:px-5 py-4">
                      <div>
                        <span className="font-medium text-foreground max-w-md leading-[1.4]">{f.question}</span>
                        <p className="text-[11px] text-muted-foreground mt-1 lg:hidden">{f.category} · {f.relatedProduct}</p>
                      </div>
                    </td>
                    <td className="px-4 md:px-5 py-4 text-muted-foreground hidden lg:table-cell">{f.category}</td>
                    ...
                    <td className="px-4 md:px-5 py-4"><StatusBadge status={f.status} /></td>
                    <td className="px-4 md:px-5 py-4 text-center tabular-nums text-muted-foreground hidden xl:table-cell">{f.order}</td>
        ...
        {filtered.length === 0 && <div className="p-12 md:p-16 text-center text-[13px] text-muted-foreground">Tidak ada FAQ ditemukan.</div>}
        {totalPages > 0 && (
