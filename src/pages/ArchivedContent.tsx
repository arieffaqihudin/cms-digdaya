    ...
    <div className="space-y-4">
      <div className="rounded-lg border border-border bg-surface p-4 shadow-card">
        <div className="relative max-w-sm">
          ...
          <Input placeholder="Search archived content..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 h-9 rounded-md" />
        </div>
      </div>
      ...
      <div className="rounded-lg border border-border bg-surface shadow-card overflow-hidden">
                ...
                <tr key={`${item.type}-${item.id}`} className="hover:bg-accent/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right">
                    <Link to={item.editUrl}>
                      <Button size="sm" variant="outline" className="h-8 rounded-md"><RotateCcw className="h-3.5 w-3.5 mr-1" /> Review</Button>
                    </Link>
                  </td>
                </tr>
              ))}
        ...
        {allArchived.length === 0 && (
          <div className="p-12 text-center text-sm text-muted-foreground">No archived content.</div>
        )}
      </div>
    </div>
  );
