...
const mockProducts: ProductItem[] = products.map((name, i) => ({
  id: `p${i + 1}`,
  name,
  ...
  return (
    <div className="space-y-4 md:space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-base md:text-lg font-semibold text-foreground">Produk</h1>
          <p className="text-[12px] md:text-[13px] text-muted-foreground mt-0.5">Kelola produk konten</p>
        </div>
        ...
        <div className="flex items-center gap-2 md:gap-3 flex-wrap">
          ...
          <Link to="/products/new">
            <Button size="sm" className={cn("h-9 rounded-[10px] text-[13px] bg-primary text-primary-foreground hover:bg-primary/90 gap-1.5", screenSize === "mobile" && "w-full")}>
              <Plus className="h-4 w-4" strokeWidth={1.6} /> Tambah Produk
            </Button>
          </Link>
        </div>
      </div>
      ...
      <div className="rounded-[12px] border border-border bg-surface shadow-card overflow-hidden">
              ...
              <div key={item.id} className="p-4 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className={cn("inline-flex items-center gap-1 rounded-full px-2 py-[2px] text-[10px] font-medium",
                      item.isActive ? "bg-[hsl(var(--status-success-bg))] text-[hsl(var(--status-success-fg))]" : "bg-muted text-muted-foreground"
                    )}>
              ...
              {filtered.map((item) => (
                <tr key={item.id} className="border-b border-border/30 hover:bg-accent/30 transition-colors duration-150">
                  ...
                  <td className="px-4 md:px-5 py-4 text-center">
                    <span className="inline-flex items-center rounded-full bg-muted px-2.5 py-[3px] text-[11px] font-medium tabular-nums text-muted-foreground">{item.count}</span>
                  </td>
                  <td className="px-4 md:px-5 py-4">
                    <span className={cn("inline-flex items-center gap-1.5 rounded-full px-2.5 py-[3px] text-[11px] font-medium",
                      item.isActive ? "bg-[hsl(var(--status-success-bg))] text-[hsl(var(--status-success-fg))]" : "bg-muted text-muted-foreground"
                    )}>
        ...
        {filtered.length === 0 && (
          <div className="p-12 md:p-16 text-center text-[13px] text-muted-foreground">Tidak ada produk ditemukan.</div>
        )}
      </div>
    </div>
  );
