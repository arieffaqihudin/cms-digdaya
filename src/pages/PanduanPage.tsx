...
import { mockGuides, products, categories } from "@/lib/mock-data";
...
export default function PanduanPage() {
  ...
  const hasActiveFilters = statusFilter !== "all" || productFilter !== "all" || categoryFilter !== "all";

  const filterSelects = (
    <>
      <Select value={productFilter} onValueChange={(v) => { setProductFilter(v); setPage(1); }}>
        <SelectTrigger className="w-full sm:w-[170px] h-9 rounded-[10px] text-[13px] border-border bg-background"><SelectValue placeholder="Produk" /></SelectTrigger>
        <SelectContent>
    ...
    <div className="space-y-4 md:space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[12px] md:text-[13px] text-muted-foreground mt-0.5">Kelola artikel panduan dan help center</p>
        </div>
      ...
      <div className="rounded-[12px] border border-border bg-surface p-3 md:p-4 shadow-card">
        <div className="flex flex-wrap items-center gap-2 md:gap-3">
          <div className="relative flex-1 min-w-[160px]">
            ...
            <Input placeholder="Cari panduan..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="pl-9 h-9 rounded-[10px] text-[13px] border-border bg-background focus-visible:ring-1 focus-visible:ring-ring" />
          </div>
          {screenSize === "mobile" ? (
            <>
              ...
              <Link to="/content/new?type=guide" className="w-full sm:w-auto">
                <Button size="sm" className="h-9 w-full sm:w-auto rounded-[10px] text-[13px] bg-primary text-primary-foreground hover:bg-primary/90 gap-1.5">
                  <Plus className="h-4 w-4" strokeWidth={1.6} /> Panduan Baru
                </Button>
              </Link>
            </>
          ) : (
      ...
      <div className="rounded-[12px] border border-border bg-surface shadow-card overflow-hidden">
        {screenSize === "mobile" ? (
          <div className="divide-y divide-border/30">
            {paged.map((g) => (
              <Link key={g.id} to={`/content/${g.id}?type=guide`} className="block p-4 hover:bg-accent/30 transition-colors">
                <p className="text-[13px] font-medium text-foreground leading-[1.4]">{g.title}</p>
                <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                  <span className="text-[11px] text-muted-foreground">{g.category}</span>
                  <span className="text-[11px] text-muted-foreground/50">·</span>
                  <span className="inline-flex items-center rounded-full bg-primary/[0.07] px-2 py-[2px] text-[10px] font-medium text-primary">{g.relatedProduct}</span>
                </div>
                <div className="flex items-center gap-2 mt-2">
                  <StatusBadge status={g.status} />
                  <span className="text-[10px] text-muted-foreground tabular-nums">{g.lastUpdated}</span>
                </div>
              </Link>
            ))}
          </div>
            ...
            <table className="w-full text-[13px]">
              <thead>
                <tr className="border-b border-border/60">
                  ...
                  <th className="px-4 md:px-5 py-3.5 text-left text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground hidden lg:table-cell">Produk Terkait</th>
                  ...
                  <th className="px-4 md:px-5 py-3.5 text-left text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground hidden xl:table-cell">Terakhir Diupdate</th>
              ...
              <tbody>
                {paged.map((g) => (
                  <tr key={g.id} className="border-b border-border/30 hover:bg-accent/30 transition-colors duration-150">
                    <td className="px-4 md:px-5 py-4">
                      <div>
                        <span className="font-medium text-foreground max-w-[320px] leading-[1.4]">{g.title}</span>
                        <p className="text-[11px] text-muted-foreground mt-1 lg:hidden">{g.category} · {g.relatedProduct}</p>
                      </div>
                    </td>
                    <td className="px-4 md:px-5 py-4 text-muted-foreground hidden lg:table-cell">{g.category}</td>
                    ...
                    <td className="px-4 md:px-5 py-4"><StatusBadge status={g.status} /></td>
                    <td className="px-4 md:px-5 py-4 text-muted-foreground tabular-nums whitespace-nowrap hidden xl:table-cell">{g.lastUpdated}</td>
        ...
        {filtered.length === 0 && <div className="p-12 md:p-16 text-center text-[13px] text-muted-foreground">Tidak ada panduan ditemukan.</div>}
        {totalPages > 0 && (
