...
import { Plus, Search, BookOpen, Pencil, Filter, Info } from "lucide-react";
...
const initialPanduan: PanduanItem[] = [
  { id: "1", topik: "Cara Membuat Surat Keluar", slug: "cara-membuat-surat-keluar", produk: "Digdaya Persuratan", createdAt: "2025-01-10", orderIndex: 0 },
  { id: "2", topik: "Panduan Arsip Digital", slug: "panduan-arsip-digital", produk: "Digdaya Persuratan", createdAt: "2025-01-12", orderIndex: 1 },
  { id: "3", topik: "Mengelola Disposisi", slug: "mengelola-disposisi", produk: "Digdaya Persuratan", createdAt: "2025-02-01", orderIndex: 2 },
  { id: "4", topik: "Setup Awal SIPD", slug: "setup-awal-sipd", produk: "Digdaya SIPD", createdAt: "2025-02-15", orderIndex: 0 },
  { id: "5", topik: "Input Anggaran", slug: "input-anggaran", produk: "Digdaya SIPD", createdAt: "2025-03-01", orderIndex: 1 },
];

export default function PanduanPage() {
  const [items, setItems] = useState<PanduanItem[]>(initialPanduan);
  const [search, setSearch] = useState("");
  const [filterProduk, setFilterProduk] = useState("all");
  const [showFilters, setShowFilters] = useState(false);
  const screenSize = useScreenSize();

  const isDragEnabled = filterProduk !== "all";
          ...
          <SortableContext items={filtered.map((i) => i.id)} strategy={verticalListSortingStrategy}>
            {screenSize === "mobile" ? (
              <div className="divide-y divide-border/30">
                {filtered.map((item) => (
                  <SortableMobileCard key={item.id} id={item.id} disabled={!isDragEnabled}>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-[13px]">
                  ...
                  <tbody>
                    {filtered.map((item) => (
                      <SortableTableRow key={item.id} id={item.id} disabled={!isDragEnabled}>
